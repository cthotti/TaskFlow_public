...
accounts_col = db["extractedaccounts"]  # NEW: { email, lastEmailTs }

def safe_filename(email): return re.sub(r'[^a-zA-Z0-9_.-]', '_', email)
...
def generate_authorization_url(email: str):
    ...
    auth_url, state = flow.authorization_url(
        access_type="offline", include_granted_scopes="true", prompt="consent", login_hint=email
    )
...
def exchange_code_for_token(state: str, code: str):
    ...
    if not mapping: raise RuntimeError("Unknown OAuth state")
    email = mapping["email"]
    ...
    accounts_col.update_one(
        {"email": email},
        {"$set": {"email": email, "lastEmailTs": None}},
        upsert=True
    )
...
# ---------- EMAIL FETCHING ----------
def is_blocked(sender): return any(b in sender.lower() for b in BLOCKED_SENDERS)
...
def fetch_latest_emails(service, email, max_results=20):
    ...
    for msg in messages:
        ...
        if is_blocked(sender): continue
        body = ""
        payload = msg_data.get("payload", {})
        if payload.get("parts"):
            for part in payload["parts"]:
                if part.get("mimeType")=="text/plain" and part.get("body",{}).get("data"):
                    body = base64.urlsafe_b64decode(part["body"]["data"]).decode("utf-8","ignore"); break
        elif payload.get("body",{}).get("data"):
            body = base64.urlsafe_b64decode(payload["body"]["data"]).decode("utf-8","ignore")
        emails.append({"subject":subject,"from":sender,"date":ts.isoformat(),
                       "date_header":date_hdr,"content":body.strip() if body else "(no content)"})
    return emails
...
def filter_new_emails(email, emails):
    ...
    for e in emails:
        ...
        if not last_seen or ts.isoformat() > last_seen:
            new_emails.append(e)
            if not max_ts or ts.isoformat() > max_ts: max_ts = ts.isoformat()
    if max_ts: 
        state_col.update_one({"email":email},{"$set":{"last_ts":max_ts}},upsert=True)
        accounts_col.update_one({"email":email},{"$set":{"lastEmailTs":max_ts}},upsert=True)  # update account
    return new_emails
...
def fetch_for_emails(email_list, max_results=20):
    ...
    for email in email_list:
        try:
            ...
            if not creds: missing_auth.append(email); continue
            ...
            print(f"Error fetching {email}: {e}"); missing_auth.append(email)
